...
class YouTubeHotFinderUnified(QMainWindow):
    ...
    def create_ytdl_tab(self):
        ...
        self.ytdl_sub_langs = QLineEdit()
        self.ytdl_sub_langs.setText("ko,en")
        self.ytdl_sub_langs.setPlaceholderText("예: ko,en (쉼표로 구분)")
        settings_layout.addWidget(self.ytdl_sub_langs, row, 1)

        settings_layout.addWidget(QLabel("자막 형식:"), row, 2)
        self.ytdl_sub_format = QComboBox()
        self.ytdl_sub_format.addItems(['best', 'srt', 'vtt', 'ass'])
        settings_layout.addWidget(self.ytdl_sub_format, row, 3)
        ...
        settings_layout.addWidget(self.ytdl_dry_run, row, 2)
    ...
    def start_ytdl_download(self):
        ...
        sub_langs = self.ytdl_sub_langs.text()
        ...
        try:
            ...
            try:
                ...
                QMessageBox.critical(self, "오류", "ytdl.py 모듈을 찾을 수 없습니다.\nyoutube/ytdl.py 파일이 있는지 확인하세요.")
                return
            ...
            self.statusBar().showMessage(f"다운로드 완료: {len(downloaded_files)}개 파일")
